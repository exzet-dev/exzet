...
TOUCHED=()
for f in $(git ls-files -- 'Cargo.toml' '*/Cargo.toml'); do
  grep -qE '^version[[:space:]]*=[[:space:]]*"' "$f" || continue
  if $DRY; then
    echo "  would bump $f"
    continue
  fi
...
for f in $(git ls-files -- 'Cargo.lock' '*/Cargo.lock'); do
  if $DRY; then
